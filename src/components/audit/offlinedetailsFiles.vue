...
import {
  ...
  getOfflineFileCount,
  getOfflineLackEnclosureFilesInfo,
...
export default {
  ...
  beforeMount(){
    setTimeout(() => {
       this.getOfflineLackEnclosureFilesInfoFun()
      }, 500)
  ...
  //归档信息
    ...
    // 获取案卷信息
    getOfflineLackEnclosureFilesInfoFun(){
      if (this.projectName == 'project') {
      this.getOfflineFileCountFun() //归档信息
        const q ={
          files_id:this.project_id,
        }
        getOfflineLackEnclosureFilesInfo(q).then(response =>{
         if (response.status === 200) {
          const { data: res, message, status } = response.data
          if (status !== 200) return this.message({ message: message, type: 'error' })
