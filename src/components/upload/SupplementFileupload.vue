...
const TAG = 'TQKJ'
  ...
  mergefileSupplement,
  unzipfileSupplement,
  bindFilefileSupplement,
  convertPdffileSupplement,
} from '@/api/supplement'
import {
  getDocumentList,
  addDocument,
        ...
        register(){//注册函数
            ...
            webUploader.Uploader.register({
                ...
                beforeSendFile:function(file){//利用md5File（）方法计算文件的唯一标记符
                    ...
                    (new webUploader.Uploader()).md5File(file,0,5*1024*1024).progress(function(percentage){
                        ...
                        const q = {
                          ...
                          // zoneTotalMd5:val
                        checkfileSupplement(q).then(response =>{
                ...
                afterSendFile:function(file){//前台通知后台合并文件
                    ...
                    //合并已经上传的切片  合并为压缩包
                    mergefileSupplement(q).then(response =>{ 
                       that.uploader.skipFile(file);
                       that.setTableBtn(file.id,'上传成功');
                       that.fList.push(response.data);
                       if (response.data.status === 200) {
                        //解压文件合并的文件
                        unzipfileSupplement({ file_id: that.fjscrow.file_id }).then(response => {
                            
                            that.setTableBtn(file.id,'正在解压文件');
                            if (response.data.status === 200) {
                                if (that.fjscrow.ztlx === 'WJJZTLX_DIC-1') {
                                    //将解压的文件绑定到指定的文件ID上 PDF形式
                                    convertPdffileSupplement({  file_id: that.fjscrow.file_id }).then(response => {
                                     that.setTableBtn(file.id,'绑定文件成功');  
                                     //清空上传队列文件 
                                    ...
                                    bindFilefileSupplement({ file_id: that.fjscrow.file_id }).then(response => {
                                        that.setTableBtn(file.id,'绑定文件成功'); 
                                        //清空文件上传队列
                                        that.uploadBtnDisabled=false;
                                        that.uploadStaus="el-icon-upload";
         ...
         setHeaders(){
            ...
            this.options.headers = {
              ...
              check_code: md5(TAG + getToken() + TIME + '/file-api/v1/file/supplement/check' + RANDOM)
            }
          },
        events(){
            var that = this;
            ...
            uplaod.on('uploadBeforeSend', function( block, data, headers ) {
                ...
                that.options.headers = {
                    ...
                    random: RANDOM,
                    expiration_time: TIME,
                    check_code: md5(TAG + getToken() + TIME + '/file-api/v1/file/supplement/upload' + RANDOM)
                }
                data.chunk = block.chunk;
                data.file_id = that.fjscrow.file_id;
