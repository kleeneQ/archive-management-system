  ...
  export default {
    ...
    const that = this
      return {
        ...
        options: {
          target: '/file-api/v1/file/audio/upload', //分片上传地址
    ...
    mounted() {
      Bus.$on('openUploaderFiles', query => { 
        ...
        this.filesType = query.filesType
        this.getAudioPictureFileListFun(query.files_id)
        this.params = query || {};
    ...
    methods: {
        ...
        this.ZIPfileListData.map((item,index) =>{
          if (item.id == id) {
            this.ZIPfileListData.splice(index,1)
             ...
             // 导入案卷ZIP包——获取案卷下的文件
        getAudioPictureFileListFun(files_id){
          const that = this
          const q = {
            page_num:1,
            page_size:1000000,
      ...
      getAudioPictureFileList(q,apiUrl).then(response => {
          if (response.status === 200 ) {
              if (response.data.data.list.length === 0) return this.message({ message: "该案卷下暂无文件,会导致导入ZIP包失败", type: 'error' })
                that.filesList = response.data.data.list
      ...
      try{
        this.filesList.forEach((item,i) =>{
                ...
                if (item.wjtm == CHfilename) {
                  file.file_id = item.file_id
                  this.filesList.splice(i,1)
            
                  throw Error()
                  // this.ZIPfileListData.push(file)
                }
              })
