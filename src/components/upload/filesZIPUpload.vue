    ...
    <div class="container">
        <div class="handle-box" >
            <div id="extend-upload-chooseFile"
                 style="float: left;margin: 0px 5px;" v-show="false">
               
             </div>
             <div class="ZIPfiles">
              <span class="fileinput-button el-button handle-del mr10 el-button--primary">
                <span><i class="el-icon-folder-add" style="margin-right:5px;"></i>选择文件夹</span>
                <input type="file" ref="inputerZIP" id="fileID" webkitdirectory multiple @change="change">
              </span>
             </div>
            <el-button
                    type="success"
            ...
            <el-button
                    ...
                    class="handle-del mr10"
                    @click="clearFiles()"
                    :disabled="clearuploadBtnDisabled"
            >清空上传队列</el-button>
            <!-- <h2 v-show="false">{{zipbin}}</h2> -->
        </div>
        <!-- <div class="showMsg">支持上传的文件后缀：{{ options.fileType }}</div> -->
        <el-table
        v-if="ZIPfjscDialogVisiblefalg"
        class="ziptable"
        max-height="500px"
        border
        :default-sort = "{prop: 'wjsx', order: 'ascending'}"
                :data="ZIPfileListData"
...
export default {
    ...
    data() {
        return {
            ...
            openZIPimport:true,//用户是否确定导入ZIP包
            dialogVisible: false,
            hpcgfiles:[], //已经合并成功的文件ZIPfjscDialogVisible
            ZIPfjscDialogVisiblefalg:false,
        }
    },
    watch: {
    ZIPfjscDialogVisibles: {
      ...
      handler(newVal, oldVal) {
        this.ZIPfjscDialogVisiblefalg = newVal
      }
    },
  },
    created (){
    ...
    destroyed: function () {
        console.group('重置上传列表完成状态===============》');
        this.ZIPfileListData.splice(0,this.ZIPfileListData.length);
        this.getDocumentListsFun();
        this.createUploader()
        this.register()
    },
    methods: {
      //导入案卷ZIP包——选择案卷ZIP包后 添加到上传队列上
        change(e){
           var inputerZIP = this.$refs.inputerZIP
           this.uploader.addFiles(inputerZIP.files) //添加文件到队列上
        },
        // 导入案卷ZIP包——获取案卷下的文件
        getDocumentListsFun(){
          const that = this
        ...
        this.ZIPfjscDialogVisibles = true
        this.ZIPfjscDialogVisiblefalg = true
        ...
        createUploader(){
            ...
            this.uploader = webUploader.create({
                ...
                file_id: this.filesList[0].file_id,
                // 单位字节，如果图片大小小于此值，不会采用压缩。512k  512*1024，如果设置为0，原图尺寸大于设置的尺寸就会压缩；如果大于0，只有在原图尺寸大于设置的尺寸，并且图片大小大于此值，才会压缩
                compressSize: 0,
                ZIPfileNumLimit : this.ZIPfileNumLimit,//验证文件总数量, 超出则不允许加入队列,默认值：undefined,如果不配置，则不限制数量
                fileSizeLimit : 100*1024*1024*1024, //1kb=1024*1024,验证文件总大小是否超出限制, 超出则不允许加入队列。
                fileSingleSizeLimit:this.ZIPfileSize, //验证单个文件大小是否超出限制, 超出则不允许加入队列。
                chunked:true,//是否开启分片上传
                threads:10,//上传并发数，可同时发送请求数
            ...
            webUploader.Uploader.register({
                ...
                beforeSendFile:function(file){//利用md5File（）方法计算文件的唯一标记符
                    ...
                    (new webUploader.Uploader()).md5File(file,0,5*1024*1024).progress(function(percentage){
                        ...
                        if (file.file_id=='' || file.file_id == undefined || file.file_id == null) {
                            deferred.reject();
                             that.fList.push(response.data);
                        }
                        const q = {
                ...
                beforeSend:function(block){//向后台发送当前文件的唯一标记，用于后台创建保存分块文件的目录
                    ...
                    (new webUploader.Uploader()).md5File(block.file,block.start,block.end).progress(function(percentage){
                        ...
                        block.zoneMd5=val;
                        if (block.file.file_id == '' || block.file.file_id == undefined) {
                            deferred.reject();
                        } else {
                          const q = {
                            chunk:block.chunk,
        ...
        mergeUploadFileFun(file) {
          ...
          mergeUploadFile(q).then(response =>{ 
            ...
            if (response.data.status === 200) {
                that.setTableBtn(file.id,response.data.message);
               
                //解压文件合并的文件
                unzipFile({ file_id: file.file_id }).then(response => {

                    if (response.data.status === 200) {
                    that.setTableBtn(file.id,response.data.message);
                    // that.BDfiles(file)
                     that.uploadBtnDisabled=false;
                    that.clearuploadBtnDisabled = false;
        ...
        BDfiles(file){
          const that = this
          that.setTableBtn(file.id,'绑定文件');  
          if (that.filesList[0].ztlx === 'WJJZTLX_DIC-1') {
                //将解压的文件绑定到指定的文件ID上 PDF形式
                convertPdfFile({ file_id: file.file_id }).then(response => {
                 that.setTableBtn(file.id,'上传成功');  
                  that.uploadBtnDisabled=false;
                  that.clearuploadBtnDisabled = false;
                that.uploadStaus="el-icon-upload";
                 that.onfjscDialogVisible()
                }).catch(error => {
                    console.log(error)
                })
                return
            } else {
                //绑定解压的文件文件指定的文件ID上  图片形式
                bindFile({ file_id: file.file_id }).then(response => {
                    that.setTableBtn(file.id,'上传成功');
                     that.uploadBtnDisabled=false;
                  that.clearuploadBtnDisabled = false;
                    that.uploadStaus="el-icon-upload"; 
                    this.message({ message: "案卷ZIP包导入完成", type: 'success' })
                    that.onfjscDialogVisible()
                }).catch(error => {
                    console.log(error)
                })
        ...
        events(){
            ...
            uplaod.on('fileQueued', function(file){
                ...
                var CHfilename = file.name.substr(0, file.name.lastIndexOf("."))
                  
                 var CHfilewjsx = file.name.substr(0, file.name.lastIndexOf("."))

                    let index = CHfilename.lastIndexOf(".")

                    CHfilewjsx = parseInt(CHfilewjsx.substr(0,index));
                    CHfilename = CHfilename.substr(index+1,CHfilename.length);
                    var progressval = ''
                // that.filesList.forEach(item =>{
                  
                // })
                 var array = that.filesList
                for (let i = 0; i < array.length; i++) {
                    const item = array[i];
                    var wjsx = item.wjsx
                    var wjtm = item.wjtm
                    if (wjtm == CHfilename && item.fjflga == false) {
                        file.file_id = item.file_id
                        progressval = '等待上传'
                        item.fjflga = true
                        break
                    }else {
                        progressval = '无法上传：附件与文件名称不符,请移除'
                        // that.uploadBtnDisabled = true
                        that.clearuploadBtnDisabled = false;
                    }
                }
                var ZIPfileSize = that.formatFileSize(file.size);
                var row={file_id:file.file_id,fileId:file.id,fileName:file.name,ZIPfileSize:ZIPfileSize,validateMd5:'0%',progress:progressval,state:'就绪',wjsx:CHfilewjsx};
                if (progressval == '等待上传') {
                    that.uploadBtnDisabled = false
                    that.clearuploadBtnDisabled = false;
                    that.ZIPfileListData.unshift(row);
                }else {
                    that.ZIPfileListData.push(row);
                }
            ...
            uplaod.on('uploadBeforeSend', function( block, data, headers ) {
                ...
                // that.fjscrow.file_id = data.file_id
                data.chunk = block.chunk;
                data.file_id = block.file.file_id;
          ...
          //  });
            ...
            uplaod.on('uploadFinished',function(){//成功后
                ...
                for (let i = 0; i < hpcgfiles.length; i++) {
                    ...
                    that.mergeUploadFileFun(element)
                }

            });
        },
        setTableBtn(fileId,showmsg,networkPath){
            var fileList=this.ZIPfileListData;
            for(var i=0;i<fileList.length;i++){
                if(fileList[i].fileId==fileId){
                    this.ZIPfileListData[i].progress=showmsg;
                    this.ZIPfileListData[i].networkPath=networkPath||"";
                }
            }
        },
        removeRow(index,row){
        ...
        ZIPImportdialogVisible(){
            this.dialogVisible = false
            this.openZIPimport = false
        },
        uploadToServer(){
            const that = this
            if(this.ZIPfileListData.length<=0){
              this.message({ message: "没有需要上传的文件", type: 'error' })
              return;  
            }
            // this.ZIPfileListData.forEach(item => { //遍历队列上的文件，对文件的附件进行删除
            //        var q = {
            //             remove_type: 2,
        ...
        clearFiles(){
            ...
            that.uploadBtnDisabled=false;
            that.clearuploadBtnDisabled = false;
            this.ZIPfjscDialogVisiblefalg = false
            that.uploadStaus="el-icon-upload";
            that.uploader.reset();
            that.ZIPfileListData.splice(0,that.ZIPfileListData.length);
            this.getDocumentListsFun();
        },
        beginUploadFile(index,row){
            this.uploadBtnDisabled=false; 
            this.clearuploadBtnDisabled = false;     
        ...
        formatFileSize(size){
            ...
            if(size/1024>1024){
                ...
                ZIPfileSize = len.toFixed(2) +"MB";
            }else if(size/1024/1024>1024){
                var len = size/1024/1024;
                ZIPfileSize = len.toFixed(2)+"GB";
            }else{
                var len = size/1024;
                ZIPfileSize = len.toFixed(2)+"KB";
            }
            return ZIPfileSize;
        },
        addFiles(){
