...
const TAG = 'TQKJ'
  ...
  import {
    ...
    netimportFile,
    supplementBindFile,
    supplementConvertPdf
  } from '@/api/netfileupload/supplementwebupload.'
import {
  getDocumentList,
  addDocument,
  editDocument,
...
export default {
    ...
    props:{
        ...
        options:{
            ...
            default:{
                ...
                mergeChunksUrl:"/file-api/v1/file/supplement/merge",//合并文件请求地址 headers:{},
                file_id:'',
                uploadSuffixUrl:'',
                headers:{},
            }
    ...
    methods: {
        ...
        createUploader(){
            console.log(this.options,"________________")
            console.log(this.options.uploadSuffixUrl,"____++++++++++++++++++____________")
                    ...
                    netmergeFile(q).then(response =>{ 
                       ...
                       if (response.data.status === 200) {
                        ...
                        netunzipFile({ file_id: file.name.substr(0, file.name.indexOf(".")) }).then(response => {
                            ...
                            console.log("正在解压文件")
                            that.setTableBtn(file.id,response.data.message);
                            supplementConvertPdf({ file_id: file.name.substr(0, file.name.indexOf(".")) }).then(response =>{
