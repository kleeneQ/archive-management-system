                ...
                afterSendFile:function(file){//前台通知后台合并文件
                    ...
                    mergeUploadFile(q).then(response =>{ 
                       ...
                       if (response.data.status === 200) { 
                        //解压文件合并的文件
                        unzipFile({ file_id: that.fjscrow.file_id }).then(response => {
                            ...
                            that.setTableBtn(file.id,response.data.message);
                                ...
                                if (that.fjscrow.ztlx === 'WJJZTLX_DIC-1') {
                                    ...
                                    convertPdfFile({  file_id: that.fjscrow.file_id }).then(response => {
                                    ...
                                    bindFile({ file_id: that.fjscrow.file_id }).then(response => {
                                        that.setTableBtn(file.id,'绑定文件成功'); 
                                        //清空文件上传队列
                                        that.uploadBtnDisabled=false;
                                        that.uploadStaus="el-icon-upload";
