...
import specialtyDetails from '@/components/audit/detailsProject'
import detailsEngineering from '@/components/details/detailsEngineering'
import specialtyDetailsFiles from '@/components/audit/detailsFiles'
export default {
  components: {
...
// 批量导入
  ...
  // 下移方法
    ...
    getSideBarFun(data) {
      ...
      getUtilizeUseEngineeringListSide(q).then(response => {
        if (response.status === 200) {
          const { data: res, message, status } = response.data
          ...
          console.log(newTress[0].children[0].treeLevel)
    ...
    searchFilesListEngineeringIdFun(){
      ...
      const q ={
        ...
        getUtilizeUseFilesListEngineeringId(q).then(response =>{
        if (response.status === 200) {
          const { data: res, message, status } = response.data
          if (status !== 200) return this.message({ message: message, type: 'error' })
    ...
    getFilesProjectListFun() {
      ...
      console.log(this.queryInfo)
      getUtilizeUseFilesListProjectId(this.queryInfo).then(response => {
    ...
    getDocumentListFun() {
      ...
      getUtilizeUseFileListFilesId(this.queryInfo).then(response => {
        if (response.status === 200) {
          const { data: res, message, status } = response.data
          if (status !== 200) return this.message({ message: message, type: 'error' })
    ...
    dialogClosed() {
      ...
      getFileUrlFun(){
        ...
        getUtilizeUseFileUrlFileId(q).then(response =>{
          if (response.status === 200) {
