        ...
        <el-col>
          <el-form :inline="true" :model="queryInfo" class="demo-form-inline"  label-width="110px">
            <el-form-item label="案卷题名">
              <el-input clearable  v-model="queryInfo.ajtm" placeholder="请输入案卷题名" @change="getSearchAudioCollectionFilesListFun"></el-input>
            </el-form-item>
            <el-form-item label="关联工程名称">
              <el-input clearable v-model="queryInfo.glgcmc" placeholder="请输入关联工程名称" @change="getSearchAudioCollectionFilesListFun"></el-input>
            </el-form-item>
            <el-form-item label="拍摄起始时间">
              <el-date-picker type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd" placeholder="选择拍摄起始时间" v-model="queryInfo.start_time" @change="getSearchAudioCollectionFilesListFun"></el-date-picker>
            </el-form-item>
             <el-form-item label="拍摄终止时间">
              <el-date-picker type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd" placeholder="选择拍摄终止时间" v-model="queryInfo.end_time" @change="getSearchAudioCollectionFilesListFun"></el-date-picker>
            </el-form-item>
            <el-form-item label="拍摄者">
              <el-input clearable v-model="queryInfo.psz" placeholder="请输入拍摄者" @change="getSearchAudioCollectionFilesListFun"></el-input>
            </el-form-item>
            <el-form-item label="拍摄地点">
              <el-input clearable v-model="queryInfo.psdd" placeholder="请输入拍摄地点" @change="getSearchAudioCollectionFilesListFun"></el-input>
            </el-form-item>
            <el-form-item label="分类号">
              <el-input clearable v-model="queryInfo.flh" placeholder="请输入分类号" @change="getSearchAudioCollectionFilesListFun"></el-input>
            </el-form-item>
            <el-form-item label="人物">
              <el-input clearable v-model="queryInfo.rw" placeholder="请输入人物" @change="getSearchAudioCollectionFilesListFun"></el-input>
            </el-form-item>
            <el-form-item label="载体类型">
              <el-select clearable  v-model="queryInfo.files_type" placeholder="请选择载体类型"  @change="getSearchAudioCollectionFilesListFun">
            ...
            <el-form-item>
              <el-button type="primary" icon="el-icon-search" @click="getSearchAudioCollectionFilesListFun">查询</el-button>
            </el-form-item>
            ...
            >
              ...
              align="left"
                ...
                <template slot-scope="scope">
                  ...
                  <el-button type="primary" size="small" v-if="scope.row.files_type == 'AJJZTLX_DIC-3'"   @click="getSearchAudioCollectionFilesPreviewFun(scope.row)">预 览</el-button>
               </template>
              </el-table-column>
            </el-table>
        ...
        <table class="mailTable" >
          ...
          <tr >
            ...
            <tr >
              ...
              <td >{{filesDetails.pssj | formatDate}}</td>
            </tr>
          <tr >
          ...
          <tr >
            ...
            <td >{{filesDetails.bzrq | formatDate}}</td>
          </tr>
          <tr >
...
<script>
import {
    getSearchAudioCollectionFilesList,
    getSearchAudioCollectionFilesPreview,
...
export default {
  ...
  mounted () {
    ...
    if (this.$route.query.name != undefined) {
      this.queryInfo.glgcmc = this.$route.query.name
    }
    this.getSecretGrade()  // 密级
    this.getStoragePeriod()  // 保管期限
    this.getSearchAudioCollectionFilesListFun() //声像档案查询获取馆藏案卷档案列表
  ...
  //文件预览
  getSearchAudioCollectionFilesPreviewFun(row){
    const that = this
    const q ={
      files_id:row.files_id
    }
    getSearchAudioCollectionFilesPreview(q).then(response =>{
    ...
    //声像档案查询获取馆藏案卷档案列表
    getSearchAudioCollectionFilesListFun(){
      this.queryInfo.page_size = this.pageSize
      this.queryInfo.page_num = this.currentPage
      getSearchAudioCollectionFilesList(this.queryInfo).then(response =>{
       if (response.status === 200) {
        const { data: res, message, status } = response.data
        if (status !== 200) return this.message({ message: message, type: 'error' })
         ...
         if (res.total !== 0 && res.list.length == 0 ) {
           this.currentPage = 1
           this.getSearchAudioCollectionFilesListFun()
         }
          res.list.map(v =>{
