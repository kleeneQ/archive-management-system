<template>
  <div class="description">
    <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>文件级查看</el-breadcrumb-item>
    </el-breadcrumb>
                ...
                <el-form :inline="true" :model="queryInfo" class="demo-form-inline" v-if="isActive === 'files'">
                  ...
                  <el-form-item label="文件档号">
                   ...
                   <el-form-item label="载体类型">
                    <el-select v-model="queryInfo.ztlx" @change="getDocumentListFun" placeholder="请选择载体类型" clearable>
                      <el-option
                   ...
                   <el-form-item label="附注">
                    <el-input clearable v-model="queryInfo.fz" placeholder="请输入附注" @change="getDocumentListFun"></el-input>
                  </el-form-item>
                  <el-form-item>
   ...
   <filesZIPUpload
    ...
    <!-- 审核详情弹层 -->
      ...
      ></AuditDetail>
        ...
        <!-- 单位工程详情 E -->
        <!-- 案卷详情 B -->
          <el-dialog :title="'文件详情'" :visible.sync="dialogprojectdetailsVisibleFiles" width="70%">
...
　　  imgsVisible: false, //默认弹窗属性是隐藏的
  ...
  created() {
    ...
    this.engineering_id = this.$route.query.project_id
    this.oneLevel = this.$route.query.project_id
  ...
  methods: {
    ...
    rowStyle({row}){
      ...
      for(let i = 0; i < arr.length; i++){
        ...
        // 案卷列表选中的列 
    changeBox(selection) {
     this.ids = selection.map(item => item.file_id);
    ...
    searchEngineeringDownloadFun(){
      ...
      const q = {
        ...
        file_ids:engineeringIds
      }
      downloadSearchFile(q).then(response =>{
        let headersName = ''
...
// 批量导入
  ...
  // 下移方法
    ...
    getSideBarFun(data) {
      ...
      searchProjectListSide(q).then(response => {
    ...
    getFilesProjectListFun() {
      ...
      console.log(this.queryInfo)
      searchFilesListProjectId(this.queryInfo).then(response => {
    ...
    getDocumentListFun() {
      ...
      searchFileList(this.queryInfo).then(response => {
        if (response.status === 200) {
          const { data: res, message, status } = response.data
          if (status !== 200) return this.message({ message: message, type: 'error' })
    ...
    dialogClosed() {
      ...
      getFileUrlFun(){
        ...
        getSearchFileUrl(q).then(response =>{
          if (response.status === 200) {
