<template>
  <div class="description">
    <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item>单位工程查看</el-breadcrumb-item>
    </el-breadcrumb>
   ...
   <filesZIPUpload
    ...
    <!-- 审核详情弹层 -->
      ...
      ></AuditDetail>
        ...
        <!-- 单位工程详情 E -->
        <!-- 案卷详情 B -->
          <el-dialog :title="detailstitle" :visible.sync="dialogprojectdetailsVisibleFiles" width="70%">
...
　　  imgsVisible: false, //默认弹窗属性是隐藏的
  ...
  created() {
    ...
    this.getLanguage()  // 文种
    this.getFilesProjectListFun() //工程列表
...
// 批量导入
  ...
  // 下移方法
    ...
    getDocumentListFun() {
      ...
      searchFileListFilesId(this.queryInfo).then(response => {
        if (response.status === 200) {
          const { data: res, message, status } = response.data
          if (status !== 200) return this.message({ message: message, type: 'error' })
