<template>
  <div >
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>光盘档案</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 头部 面包屑导航 E -->
        ...
        <el-col>
          <el-form :inline="true" :model="queryInfo" class="demo-form-inline">
            <el-form-item label="案卷题名">
              <el-input clearable v-model="queryInfo.ajtm" placeholder="请输入案卷题名" @change="getAudioCdFilesListFun"></el-input>
            </el-form-item>
            <el-form-item label="关联工程名称">
              <el-input clearable v-model="queryInfo.glgcmc" placeholder="请输入关联工程名称" @change="getAudioCdFilesListFun"></el-input>
            </el-form-item>
            <el-form-item>
              <el-button type="primary" icon="el-icon-search" @click="getAudioCdFilesListFun">查询</el-button>
            </el-form-item>
             <el-form-item>
            ...
            >
              ...
              align="center"
                ...
                <template slot-scope="scope">
                  ...
                  <el-button type="danger" size="small"     @click="deleteCdFilesFun(scope.row)">删除</el-button>
                  <el-button type="primary" size="small"    @click="gotoaudioPictureFilePageFun(scope.row)">查看</el-button>
                  <!-- <el-button type="primary" size="small" v-if="scope.row.files_type == 'AJJZTLX_DIC-4'"   @click="getAudioCollectionFilesPreviewFun(scope.row)">预览</el-button> -->
               </template>
              </el-table-column>
            </el-table>
    ...
    <!-- 添加声像档案 弹层 B -->
      ...
      <el-form class="catalogueengineering" :inline="true" :model="filesPictureForm" ref="filesPictureFormref" :rules="filesPictureFormRules" label-width="100px">
        ...
        <el-form-item class="btn">
          ...
          <el-button type="primary" v-if="titleName=='添加声像档案'" @click="addAudioCdFilesFun">确 定</el-button>
          <el-button type="primary" v-if="titleName!=='添加声像档案'" @click="updateAudioCdFilesFun">确 定</el-button>
       
          <!-- <el-button type="primary" @click="addAudioCdFilesFun">确 定</el-button> -->
        </el-form-item>
      </el-form>
...
export default {
  ...
  mounted () {
    this.getAudioCdFilesListFun() //获取档案列表 数据
...
destroyed() {
  ...
  if (this.filesPictureForm.pssj !== undefined) {
    ...
    this.filesPictureForm.files_type = 'AJJZTLX_DIC-4'

    updateAudioCdFiles(this.filesPictureForm).then(response =>{
     if (response.status === 200) {
      ...
      this.getAudioCdFilesListFun() //刷新档案列表
...
addAudioCdFilesFun(){
  ...
  addAudioCdFiles(this.filesPictureForm).then(response =>{
   if (response.status === 200) {
    const { data: res, message, status } = response.data
    if (status !== 200) return this.message({ message: message, type: 'error' })
  ...
  // 获取档案列表 数据
    getAudioCdFilesListFun(){
      const q ={
        ajtm:this.queryInfo.ajtm,
        glgcmc:this.queryInfo.glgcmc,
        page_num:this.currentPage,
        page_size:this.pageSize,
        files_type:'AJJZTLX_DIC-4',
      }
      getAudioCdFilesList(q).then(response =>{
       if (response.status === 200) {
        const { data: res, message, status } = response.data
        if (status !== 200) return this.message({ message: message, type: 'error' })
