<template>
  <div >
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>录像档案</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 头部 面包屑导航 E -->
        ...
        <el-col>
          <el-form :inline="true" :model="queryInfo" class="demo-form-inline">
            <el-form-item label="案卷题名">
              <el-input clearable v-model="queryInfo.ajtm" placeholder="请输入案卷题名" @change="getAudiovideoFilesListFun"></el-input>
            </el-form-item>
            <el-form-item label="关联工程名称">
              <el-input clearable v-model="queryInfo.glgcmc" placeholder="请输入关联工程名称" @change="getAudiovideoFilesListFun"></el-input>
            </el-form-item>
            <el-form-item>
              <el-button type="primary" icon="el-icon-search" @click="getAudiovideoFilesListFun">查询</el-button>
            </el-form-item>
             <el-form-item>
            ...
            >
              ...
              align="center"
                ...
                <template slot-scope="scope">
                  ...
                  <el-button type="danger" size="small"     @click="deletevideoFilesFun(scope.row)">删除</el-button>
                  <el-button type="primary" size="small"    @click="gotoaudioPictureFilePageFun(scope.row)">查看</el-button>
                  <!-- <el-button type="primary" size="small" v-if="scope.row.files_type == 'AJJZTLX_DIC-5'"   @click="getAudioCollectionFilesPreviewFun(scope.row)">预览</el-button> -->
               </template>
              </el-table-column>
            </el-table>
    ...
    <!-- 添加声像档案 弹层 B -->
      <el-dialog title="添加声像档案" :visible.sync="dialogfilesPictureFormVisible" width="65%">
      <el-form class="catalogueengineering" :inline="true" :model="filesPictureForm" ref="filesPictureFormref" :rules="filesPictureFormRules" label-width="110px">
      ...
      <el-form-item label="分类号: " prop="flh">
        ...
        <el-form-item class="btn">
          ...
          <el-button type="primary" v-if="titleName=='添加声像档案'" @click="addAudiovideoFilesFun">确 定</el-button>
          <el-button type="primary" v-if="titleName!=='添加声像档案'" @click="updateAudiovideoFilesFun">确 定</el-button>
       
          <!-- <el-button type="primary" @click="addAudiovideoFilesFun">确 定</el-button> -->
        </el-form-item>
      </el-form>
...
<script>
import {
  getAudiovideoFilesList, 
  addAudiovideoFiles,
  getAudioProjectList,
  deletevideoFiles,
  getAudioCollectionFilesPreview,
  updateAudiovideoFiles,
...
export default {
  ...
  mounted () {
    this.getAudiovideoFilesListFun() //获取档案列表 数据
...
destroyed() {
  ...
  if (this.filesPictureForm.pssj !== undefined) {
    ...
    this.filesPictureForm.files_type = 'AJJZTLX_DIC-5'

    updateAudiovideoFiles(this.filesPictureForm).then(response =>{
     if (response.status === 200) {
      ...
      this.getAudiovideoFilesListFun() //刷新档案列表
  ...
  deletevideoFilesFun(row){
   ...
   this.confirm('是否删除该题名下所有照片档案信息么？', '提示', {
        ...
        type: 'warning'
            ...
            deletevideoFiles(q).then(response =>{
            if (response.status === 200) {
              const { data: res, message, status } = response.data
              if (status !== 200) return this.message({ message: message, type: 'error' })
...
addAudiovideoFilesFun(){
  ...
  addAudiovideoFiles(this.filesPictureForm).then(response =>{
   if (response.status === 200) {
    const { data: res, message, status } = response.data
    if (status !== 200) return this.message({ message: message, type: 'error' })
  ...
  // 获取档案列表 数据
    getAudiovideoFilesListFun(){
      const q ={
        ajtm:this.queryInfo.ajtm,
        glgcmc:this.queryInfo.glgcmc,
        page_num:this.currentPage,
        page_size:this.pageSize,
        files_type:'AJJZTLX_DIC-5',
      }
      getAudiovideoFilesList(q).then(response =>{
       if (response.status === 200) {
        const { data: res, message, status } = response.data
        if (status !== 200) return this.message({ message: message, type: 'error' })
         ...
         if (res.total !== 0 && res.list.length == 0 ) {
           this.currentPage = 1
           this.getAudiovideoFilesListFun()
         }
