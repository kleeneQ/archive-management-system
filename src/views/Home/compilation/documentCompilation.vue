<template>
  <div >
        ...
        <el-breadcrumb-item>文件汇编</el-breadcrumb-item>
    </el-breadcrumb>
     <el-card>
    ...
    <!-- 新增编研管理 - 文件汇编 弹层 B -->
    <el-dialog :title="dialogcompilationFormVisibletitle+'文件汇编'" :visible.sync="dialogcompilationFormVisible" width="70%" @close='savecompilationFormFuntwoX'>
    <el-form class="Medicate" :inline="true" :model="compilationForm" ref="compilationForm" :rules="compilationFormrules" label-width="150px">
      <el-form-item label="文件汇编名称: " prop="compilation_title" class="Medicate-title">
        <el-input clearable v-model="compilationForm.compilation_title" placeholder="请输入文件汇编名称"></el-input>
      </el-form-item>
...
<script>
import {
  getCompilationdocumentCompilationList,
  getCompilationdocumentCompilationSave,
  getCompilationdocumentCompilationListInfo,
  getCompilationdocumentCompilationDelete,
  getCompilationdocumentCompilationUpdate,
...
export default {
  ...
  data () {
    return {
      ...
      currentPage: 1, // 当前页数
      pageSize: 10,  // 每页显示的条数
      queryInfo: { //获取编研管理 - 文件汇编列表参数
        page_num:1,
        page_size:10,
        tree_id:'0',
      },
      compilationList:[],//编研管理 - 文件汇编列表数据
      dialogcompilationFormVisible:false,//新增编辑编研管理 - 文件汇编  弹层
      compilationForm:{ //新增编辑编研管理 - 文件汇编 表单
        compilation_title:'',
      ...
      dialogcompilationFormVisibletitle:'', //编辑 新增 编研管理 - 文件汇编 标题
      compilationFormrules:{ //编辑 新增 编研管理 - 文件汇编  校验规则
        compilation_title: [
            { required: true, message: '请输入文件汇编名称', trigger: 'blur' },
          ],
        compilation_abstract: [
      ...
      compilation_id:'',//
      fileflag:false,//是否需要保存 文件汇编信息
    }
  },
  mounted () {
    // this.getInfoFun() //获取用户信息
    this.getCompilationdocumentCompilationListFun() //获取编研管理 - 文件汇编列表
  },
  methods: {
    // getCompilationdocumentCompilationListInfoFun(compilation_id){
    //   const q ={
    //     compilation_id:compilation_id
    //   }
    //   getCompilationdocumentCompilationListInfo().then(response =>{
    ...
    UpdatecompilationFormFun(formName){
          ...
          getCompilationdocumentCompilationUpdate(this.compilationForm).then(response =>{
            
            if (response.status === 200) {
            const { data: res, message, status } = response.data
            if (status !== 200) return this.message({ message: message, type: 'error' })
            this.message({ message: message, type: 'success' })
            this.getCompilationdocumentCompilationListFun()
            this.dialogcompilationFormVisible = false
              this.$refs.upload.clearFiles();
            }
          })
    },
    //如果 取消 则 删除 新添加的最新的文件汇编  ID 
    savecompilationFormFuntwoX(){
      ...
      if( this.fileflag == false) return 
        getCompilationdocumentCompilationDelete(q).then(response => {
              if (response.status === 200) {
              const { data: res, message, status } = response.data
              if (status !== 200) return this.message({ message: message, type: 'error' })
    ...
    //保存附件到 对应的文件汇编条上   如果保存新的数据的话，就将  数据更新到 最新的文件汇编ID 上
    ...
    savecompilationFormFun(formName){
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          getCompilationdocumentCompilationSave(this.compilationForm).then(response =>{
    ...
    myUpload(e){
      ...
      getCompilationdocumentCompilationSave(this.compilationForm).then(response =>{
        ...
        if (status !== 200) return this.message({ message: message, type: 'error' })
        this.fileflag = true
      //  this.getCompilationdocumentCompilationListFun() //保存新的 文件汇编  并且刷新列表，获取刚保存的 最新的 文件汇编 ID
       const qqq = {
          page_num:1,
          page_size:10,
          compilation_type:'6',
       }
       getCompilationdocumentCompilationList(qqq).then(response =>{
        if (response.status === 200) {
        const { data: res, message, status } = response.data
        if (status !== 200) return this.message({ message: message, type: 'error' })
    ...
    //获取编研管理 - 文件汇编列表
    getCompilationdocumentCompilationListFun(){
      const q = {
        page_num:this.currentPage,
        page_size:this.pageSize,
        compilation_type:'6',
      }
      getCompilationdocumentCompilationList(q).then(response =>{
        if (response.status === 200) {
        const { data: res, message, status } = response.data
        if (status !== 200) return this.message({ message: message, type: 'error' })
