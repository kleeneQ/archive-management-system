<template>
  <div >
        ...
        <el-breadcrumb-item>案卷分类目录</el-breadcrumb-item>
    </el-breadcrumb>
     <el-card>
    ...
    <!-- 新增编研管理 - 案卷分类目录 弹层 B -->
    <el-dialog :title="dialogcompilationFormVisibletitle+'案卷分类目录'" :visible.sync="dialogcompilationFormVisible" width="70%" @close='savecompilationFormFuntwoX'>
    <el-form class="Medicate" :inline="true" :model="compilationForm" ref="compilationForm" :rules="compilationFormrules" label-width="150px">
      <el-form-item label="案卷分类目录名称: " prop="compilation_title" class="Medicate-title">
        <el-input clearable v-model="compilationForm.compilation_title" placeholder="请输入案卷分类目录名称"></el-input>
      </el-form-item>
...
<script>
import {
  getCompilationfilesClassifyCatalogueList,
  getCompilationfilesClassifyCatalogueSave,
  getCompilationfilesClassifyCatalogueDelete,
  getCompilationfilesClassifyCatalogueUpdate,
...
export default {
  ...
  data () {
    return {
      ...
      currentPage: 1, // 当前页数
      pageSize: 10,  // 每页显示的条数
      queryInfo: { //获取编研管理 - 案卷分类目录列表参数
        page_num:1,
        page_size:10,
        tree_id:'0',
      },
      compilationList:[],//编研管理 - 案卷分类目录列表数据
      dialogcompilationFormVisible:false,//新增编辑编研管理 - 案卷分类目录  弹层
      compilationForm:{ //新增编辑编研管理 - 案卷分类目录 表单
        compilation_title:'',
      ...
      dialogcompilationFormVisibletitle:'', //编辑 新增 编研管理 - 案卷分类目录 标题
      compilationFormrules:{ //编辑 新增 编研管理 - 案卷分类目录  校验规则
        compilation_title: [
            { required: true, message: '请输入案卷分类目录名称', trigger: 'blur' },
          ],
        compilation_abstract: [
      ...
      compilation_id:'',//
      fileflag:false,//是否需要保存 案卷分类目录信息
    }
  },
  mounted () {
    // this.getInfoFun() //获取用户信息
    this.getCompilationfilesClassifyCatalogueListFun() //获取编研管理 - 案卷分类目录列表
  },
  methods: {
    // getCompilationfilesClassifyCatalogueListInfoFun(compilation_id){
    //   const q ={
    //     compilation_id:compilation_id
    //   }
    //   getCompilationfilesClassifyCatalogueListInfo().then(response =>{
    ...
    UpdatecompilationFormFun(formName){
          ...
          getCompilationfilesClassifyCatalogueUpdate(this.compilationForm).then(response =>{
            
            if (response.status === 200) {
            const { data: res, message, status } = response.data
            if (status !== 200) return this.message({ message: message, type: 'error' })
            this.message({ message: message, type: 'success' })
            this.getCompilationfilesClassifyCatalogueListFun()
            this.dialogcompilationFormVisible = false
              this.$refs.upload.clearFiles();
            }
          })
    },
    //如果 取消 则 删除 新添加的最新的案卷分类目录  ID 
    savecompilationFormFuntwoX(){
      ...
      if( this.fileflag == false) return 
        getCompilationfilesClassifyCatalogueDelete(q).then(response => {
              if (response.status === 200) {
              const { data: res, message, status } = response.data
              if (status !== 200) return this.message({ message: message, type: 'error' })
    ...
    //保存附件到 对应的案卷分类目录条上   如果保存新的数据的话，就将  数据更新到 最新的案卷分类目录ID 上
    ...
    savecompilationFormFun(formName){
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          getCompilationfilesClassifyCatalogueSave(this.compilationForm).then(response =>{
    ...
    myUpload(e){
      ...
      getCompilationfilesClassifyCatalogueSave(this.compilationForm).then(response =>{
        ...
        if (status !== 200) return this.message({ message: message, type: 'error' })
        this.fileflag = true
      //  this.getCompilationfilesClassifyCatalogueListFun() //保存新的 案卷分类目录  并且刷新列表，获取刚保存的 最新的 案卷分类目录 ID
       const qqq = {
          page_num:1,
          page_size:10,
          compilation_type:'4',
       }
       getCompilationfilesClassifyCatalogueList(qqq).then(response =>{
        if (response.status === 200) {
        const { data: res, message, status } = response.data
        if (status !== 200) return this.message({ message: message, type: 'error' })
    ...
    //获取编研管理 - 案卷分类目录列表
    getCompilationfilesClassifyCatalogueListFun(){
      const q = {
        page_num:this.currentPage,
        page_size:this.pageSize,
        compilation_type:'4',
      }
      getCompilationfilesClassifyCatalogueList(q).then(response =>{
        if (response.status === 200) {
        const { data: res, message, status } = response.data
        if (status !== 200) return this.message({ message: message, type: 'error' })
