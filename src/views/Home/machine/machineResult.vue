    ...
    <div class="resultbox">
      ...
      <div class="resultboxtop" v-else>
       <!-- 搜索栏 B -->
        <el-row  style="margin-bottom: 20px;background-color: #fff;padding:20px;">
            <el-col>
              <el-form :inline="true"  ref="queryInfo" :model="queryInfo" :rules="rules" class="demo-form-inline" key="two">
      ...
      <div class="resultboxtable" v-if="files_type == 'files_type1'">
           ...
           <el-table-column
              ...
              <el-table-column
                ...
                width="100">
                <template slot-scope="scope">
                  <el-button v-if="scope.row.zt == 'file_status3'"  size="small" class="btnedit" @click="gotofilespage(scope.row)">查看档案</el-button>
                </template>
              </el-table-column>
            </el-table>
            <!-- 工程列表 E -->
  ...
  //案卷归属类别 工程类或管理类
    ...
    getMachineUtilizeIntegratedMachineRecordListFun(){
      this.$refs.queryInfo.validate((valid) => {
        if (valid) {
             console.log(this.files_type,'123')
      let cdlx = '2'
      ...
      getMachineUtilizeIntegratedMachineRecordList(this.queryInfo).then(response =>{
       if (response.status === 200) {
