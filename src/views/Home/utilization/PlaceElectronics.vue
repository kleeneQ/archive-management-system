<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>电子档案管理</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 头部 面包屑导航 E -->
    <!-- 检索栏 B -->
...
export default {
  ...
  mounted () {
    ...
    this.getEngineeringTypeFun() //获取项目类型
     setTimeout(() => {
    this.getUtilizationPlaceElectronicsListFun() //获取项目列表
      }, 500)
  },
  methods: {
      //归档信息
    ...
    dialogPlaceVisibleFun(name,row){
      ...
      this.$confirm('确定将该项目归档么？', '提示', {
          ...
          UtilizationElectronicsPaper(q).then(response =>{
            if (response.status === 200) {
            const { data: res, message, status } = response.data
            if (status !== 200) return this.message({ message: message, type: 'error' })
            this.message({ message: message, type: 'success' })
            
            this.getUtilizationPlaceElectronicsListFun()
            }
          })
        }).catch(() => {
          this.message({
    ...
    // 获取项目档案列表
    getUtilizationPlaceElectronicsListFun(){
      const q = {
      ...
      getUtilizationPlaceElectronicsList(q).then(response =>{
        if (response.status === 200) {
        const { data: res, message, status } = response.data
        if (status !== 200) return this.message({ message: message, type: 'error' })
